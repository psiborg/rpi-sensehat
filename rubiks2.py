...
def scramble_cube(moves=10):
    """Scramble the cube by randomly rotating faces."""
...
def joystick_event(event):
    ...
    if event.action == 'pressed':
        if event.direction == 'up':
            rotate_view('up')  # Rotate view upward
        elif event.direction == 'down':
            rotate_view('down')  # Rotate view downward
        elif event.direction == 'left':
            rotate_view('left')  # Rotate view to the left
        elif event.direction == 'right':
            rotate_view('right')  # Rotate view to the right
        elif event.direction == 'middle':
            # Rotate current front face by default (could change to rotate any face)
            rotate_face(view[2], 'clockwise')
        display_cube()

# Allow players to rotate any face by using buttons
def rotate_any_face(face, direction):
    rotate_face(face, direction)
    display_cube()
...
# Scramble the cube before starting
scramble_cube(moves=5)
